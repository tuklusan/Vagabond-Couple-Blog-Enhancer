#!/usr/bin/env python3
"""
Shared LLM client for the Vagabond-Couple-Blog-Enhancer offload pipeline.

Primary provider:  OpenRouter (free model)  -> minimises Claude-Code cloud usage
Fallbacks:         DeepSeek, then NVIDIA NIM (reuse existing keys/infra)

Every offload script (review gate, doc-check, code agent, test gen/analysis)
imports from here so there is exactly ONE place that talks to the models.
...
def _load_key(env_var: str, secret_file: str, prefix: str) -> str:
    """Load a key from an environment variable, else a KEY=VALUE secrets file."""
...
def chat(messages, max_tokens=2048, temperature=0.1, allow_fallback=True):
    """
    Send a chat completion. OpenRouter first (free, minimises Claude usage);
    on failure fall back to DeepSeek then NVIDIA so the pipeline keeps moving.
    ...
    try:
        ...
        if not allow_fallback:
            raise
        safe_print("[or_client] OpenRouter failed (" + str(e) + "); trying DeepSeek...")

    try:
        return call_deepseek(messages, max_tokens, temperature), "deepseek-v4-pro"
    except Exception as e:
        errors.append("deepseek=" + str(e))
        safe_print("[or_client] DeepSeek failed (" + str(e) + "); trying NVIDIA...")
