...
def _document_review(html, context=None, state=None):
    ...
    system = (
        ...
        "repeats across sections. Flag repetition regardless of whether it feels like "
        "original author wording or added wording -- you are judging the reading experience "
        "of the post as it exists now, not who wrote which sentence; (e) SMOOTH READ -- as "
        "you read straight through, does the experience ever feel disrupted? Specifically: "
        "does the TIMELINE oscillate (jumping backward/forward, or restating something as if "
        "for the first time after it already happened)? Does the GEOGRAPHIC ROUTE oscillate "
        "(place names appearing out of travel order, or a location revisited with no "
        "explanation of returning there)? Watch especially for WHIPLASH -- the narrative "
        "moves on to a later stop/waypoint, then briefly jumps back to an earlier stop, then "
        "resumes forward again, with no flashback framing that makes the backward jump "
        "intentional. Are there jarring transitions, a shift in authorial voice, or content "
        "that appears AFTER the post's own closing/sign-off line (e.g. after 'The End.', "
        "'Until next time', or similar) as if the post kept going after it already said "
        "goodbye?"
        + series_note +
